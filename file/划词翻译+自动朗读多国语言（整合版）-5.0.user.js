...
// @author       𝓌𝑜𝒷
// @version      5.0
// @description  ♻采最全语言检测逻辑，智能分流：纯英文单词走有道，其他语言和句子走wobs-translate。✨划词后自动朗读，若电脑缺少语音包则提示，Ctrl+空格 切换。
// @match        *://*/*
...
(function () {
  ...
  function handleMouseUp() {
    ...
    if (isLikelyEnglishWord) {
      fetchYoudaoTranslation(text, rect, speechSuccess);
    } else {
      fetchWobTranslation(text, sourceLang, rect, speechSuccess);
    }
  ...
  // --- wob 翻译请求函数 ---
  function fetchWobTranslation(text, sourceLang, rect, speechSuccess) {
    if (!scriptEnabled) return;
  ...
  // --- 有道词典翻译请求函数 ---
  function fetchYoudaoTranslation(word, rect, speechSuccess) {
    if (!scriptEnabled) return;
    GM_xmlhttpRequest({
      method: 'GET',
